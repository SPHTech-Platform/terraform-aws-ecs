locals {
  log_group_name = var.log_group_name == "" ? "/aws/ecs/${var.ecs_cluster_name}" : var.log_group_name
  asg_cap_name   = format("asg-%s", var.ecs_cluster_name)
}
...
resource "aws_ecs_capacity_provider" "this" {
  count = var.create_ecs_asg_capacity_provider ? 1 : 0
